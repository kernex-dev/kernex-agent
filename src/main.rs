...
// All module declarations now live in `src/lib.rs`. main.rs imports
// the library's public surface so existing `cli::`, `commands::`,
// `mem::`, etc. paths continue to resolve. The binary entry point
// remains here; integration tests reach the same modules via the
// library crate path (e.g. `use kernex_agent::mem::cli::*`).
#[cfg(feature = "memory-cli")]
use kernex_agent::mem;
use kernex_agent::{
    api_key_var, build_provider, builtins, cli, commands, config, context_needs, data_dir_for,
    loader, prompts, scheduler, skills, stack, CliHookRunner, ProviderFlags, PROVIDERS,
};
...
use cli::{Cli, Command, CronAction, PipelineAction, SkillsAction};
use commands::CommandResult;
#[cfg(feature = "serve")]
use kernex_agent::serve::cmd_serve;
