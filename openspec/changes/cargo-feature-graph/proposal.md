...
- **The `serve` feature must not silently leak `axum` into the minimal variant.** Mitigation: explicit `cargo tree -e normal --no-default-features --features memory-cli` audit during verification; the audit output is committed under `docs/dep-tree-minimal-2026-05-10.txt` for the PR description and future regression checks. CI's `dep-tree-audit` job hard-fails if any of `axum`, `hmac`, `subtle`, `uuid`, `rusqlite`, `ratatui`, `crossterm` appears in the minimal dep tree.
...
### Finding 2. `toml_edit` is structurally pulled, not adapter-specific

`toml_edit v0.22.27` is a transitive dep of `toml v0.8.23`. `toml` is a direct dep of `kernex-agent`, `kernex-core`, `kernex-memory`, `kernex-pipelines`, and `kernex-runtime`. Gating `toml_edit` behind `agent-codex` does not remove it from the default or minimal binary; the Cargo declaration is documentation-only for size purposes today.

- **Mitigation for this change.** Keep `agent-codex = ["dep:toml_edit"]` as an intent marker. Add an inline comment in `kernex-agent/Cargo.toml` next to the line: `# Intent marker: toml_edit is currently pulled transitively via toml; this gate has zero size effect today.`
- **Follow-up.** When `toml` itself becomes a candidate for gating (a workspace-wide refactor), promote `toml_edit` from intent marker to actual opt-in. Track the dep tree at each major release.
